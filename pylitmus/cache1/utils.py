...
def _delete_one(store: typing.Callable, item_key: EntityKey):
...
def _get_counter_one(store: typing.Callable, item_key: EntityKey) -> int:
...
def _get_one(store: typing.Callable, item_key: EntityKey) -> typing.Any:
...
def _get_one_from_many(store: typing.Callable, item_key: EntityKey) -> typing.Any:
...
def _set_one(store: typing.Callable, entity: Entity) -> str:
    """Set item under a key.
    
    """
    store.set(entity.key, entity.data, ex=entity.expiration)

    return entity.key


def _set_one_singleton(store: typing.Callable, entity: Entity) -> typing.Tuple[str, bool]:
    """Sets item under a key if not already cached.
    
    """
    key, was_cached = entity.key, bool(store.setnx(entity.key, entity.data_as_json))
    if was_cached and entity.expiration:
        store.expire(key, entity.expiration)
...
# A real world assets has to settle in the real legal world.
# Tech <-> Legal ... spaces need to be aligned.
